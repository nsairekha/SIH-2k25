...
exports.deleteSurvey = exports.getSurveyAnalytics = exports.getSurveyTemplates = exports.submitSurveyResponses = exports.getSurveyById = exports.getSurveys = exports.createSurvey = void 0;
...
const createSurvey = async (req, res) => {
    try {
        ...
        const survey = new Survey_1.default(surveyData);
        await survey.save();
        res.status(201).json({
            success: true,
...
const getSurveys = async (req, res) => {
    try {
        ...
        const surveys = await Survey_1.default.find(query)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(parseInt(limit))
            .lean();
        const total = await Survey_1.default.countDocuments(query);
        res.json({
            success: true,
            data: {
                surveys,
...
const getSurveyById = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user._id;
        const survey = await Survey_1.default.findOne({ _id: id, userId });
        if (!survey) {
...
const submitSurveyResponses = async (req, res) => {
    try {
        ...
        const providedQuestionIds = responses.map((r) => r.questionId);
        ...
        survey.responses = responses.map((response) => ({
...
const getSurveyTemplates = async (req, res) => {
    try {
        ...
        if (type && templates[type]) {
            selectedTemplate = { [type]: templates[type] };
        }
        res.json({
            success: true,
...
const getSurveyAnalytics = async (req, res) => {
    try {
        ...
        const query = { userId, isCompleted: true, ...dateFilter };
        if (type) {
            query.type = type;
        }
        const surveys = await Survey_1.default.find(query).lean();
...
exports.getSurveyAnalytics = getSurveyAnalytics;
const deleteSurvey = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user._id;
        const survey = await Survey_1.default.findOneAndDelete({ _id: id, userId });
        if (!survey) {
            return res.status(404).json({
                success: false,
                message: 'Survey not found'
